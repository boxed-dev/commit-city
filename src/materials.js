...
export function buildingMaterial() {
  ...
  mat.onBeforeCompile = (shader) => {
    shader.uniforms.uNight = { value: 1.0 };          // 1 = night, 0 = day
    ...
    shader.vertexShader =
      ...
      shader.vertexShader.replace(
        ...
        `#include <begin_vertex>
         ...
         // local-ish coords: instance scale applied, translation via seed offset keeps
         // window pattern stable per building regardless of world position
         vLPos = position * vec3(1.0);
      ...
      shader.fragmentShader
        .replace('#include <map_fragment>',
          `#include <map_fragment>
           ...
           base *= 1.0 + (1.0 - uNight) * 2.8;        // daylight albedo lift
        ...
        .replace('#include <emissivemap_fragment>',
          `#include <emissivemap_fragment>
           ...
           totalEmissiveRadiance += pane * on * wc * 1.15 * flick * uNight;
           // ground-floor storefront band: lit but not nuclear
           float storefront = step(vLPos.y, 3.2) * step(0.5, vLPos.y) * pane;
           totalEmissiveRadiance += storefront * vec3(1.0,0.62,0.28) * 0.55 * uNight;
           // daytime: windows read as sky-tinted glass instead
           float dayGlassArea = pane * (vInfo.z < 0.34 ? 1.0 : 0.55);
           totalEmissiveRadiance += dayGlassArea * vec3(0.45,0.6,0.75) * 0.35 * (1.0 - uNight);`);
  };
  return mat;
}

// --- ONE road tile (pitchX × pitchZ), tiled infinitely via RepeatWrapping ---
// Block pad CENTERED in the tile, road corridors on the tile boundaries.
// The pattern is mirror-symmetric, so texture flips can't misalign roads.
export function roadTile(cfg) {
  ...
  // asphalt with faint patch noise
  ...
  g.fillStyle = '#31374a'; g.fillRect(bx - m, bz - m, bw + 2 * m, bd + 2 * m);   // sidewalk
  ...
  g.fillStyle = '#1c202e'; g.fillRect(bx, bz, bw, bd);                            // lot

  // corridor centres sit on tile edges — draw markings at both edges (wraps seamlessly)
  ...
  for (const e of [0, W]) { lineV(e - S(0.55), '#e0b83d', S(0.45)); lineV(e + S(0.55), '#e0b83d', S(0.45)); }
  for (const e of [0, H]) { lineH(e - S(0.55), '#e0b83d', S(0.45)); lineH(e + S(0.55), '#e0b83d', S(0.45)); }
  const laneDash = [S(2.6), S(3.6)], laneCol = 'rgba(230,237,250,.62)';
  for (const e of [0, W]) { lineV(e - S(road / 4), laneCol, S(0.32), laneDash); lineV(e + S(road / 4), laneCol, S(0.32), laneDash); }
  for (const e of [0, H]) { lineH(e - S(road / 4), laneCol, S(0.32), laneDash); lineH(e + S(road / 4), laneCol, S(0.32), laneDash); }

  // crosswalks on every corridor approach (symmetric: both edges, both sides)
  g.fillStyle = 'rgba(230,237,250,.68)';
  const band = S(2.6), strip = S(0.7), step = S(1.15);
  for (let i = -3; i <= 3; i++) {
    for (const ex of [0, W]) {                     // vertical corridors at x = 0 / W
      g.fillRect(ex + i * step - strip / 2, bz - m - band - S(0.4), strip, band);
      g.fillRect(ex + i * step - strip / 2, bz + bd + m + S(0.4), strip, band);
    }
    for (const ez of [0, H]) {                     // horizontal corridors at z = 0 / H
...
export function skyDome() {
  const mat = new THREE.ShaderMaterial({
    ...
    uniforms: {
      top: { value: new THREE.Color(0x04050d) },
      horizon: { value: new THREE.Color(0x101336) },
      glow: { value: new THREE.Color(0x2c1b52) },
    },
    ...
    fragmentShader: `
      ...
      void main(){
        ...
        vec3 c = mix(horizon, top, smoothstep(0.5, 1.0, h));
        c = mix(c, glow, smoothstep(0.53, 0.44, h) * 0.6);
        gl_FragColor = vec4(c, 1.0);
      }`,
  });
  return new THREE.Mesh(new THREE.SphereGeometry(1600, 24, 16), mat);
}
